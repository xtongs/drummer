  ...
  try {
    ...
    return output
      ...
      .some(
        (line) =>
          line.trim() &&
          line.includes("openspec/changes") &&
          line.includes("proposal.md"),
      );
  } catch {
    return false;
  }
...
function main() {
  ...
  if (!hasProposal) {
    ...
    log(
      COLORS.reset,
      "   openspec/changes/YYYY-MM-DD-feature-name/proposal.md",
    );
    log(
      COLORS.reset,
  ...
  log(
    COLORS.reset,
    hasProposal ? "   ✅ 规范文档: 已创建" : "   ⚠️  规范文档: 缺失",
  );
  log(
    COLORS.reset,
    testResult.passed ? "   ✅ 测试状态: 通过" : "   ❌ 测试状态: 失败",
  );
