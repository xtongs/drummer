...
function readPackageVersion() {
  try {
    // 尝试从 git HEAD 读取
    const headPackage = execSync('git show HEAD:package.json', { encoding: 'utf-8' });
...
function main() {
  ...
  log(COLORS.yellow, `${levelEmojis[requiredLevel]} 需要 ${requiredLevel} 级别递增`);
  log(COLORS.reset, `   ${levelNames[requiredLevel]}`);
  log(COLORS.cyan, `   预期版本: ${expectedVersion}\n`);

  // 检查 package.json 是否已修改
  try {
    const gitStatus = execSync('git status --porcelain package.json', { encoding: 'utf-8' });
    const isPackageStaged = gitStatus.trim().length > 0 &&
                           (gitStatus.includes('M') || gitStatus.includes('A'));

    if (isPackageStaged) {
      // 读取暂存区的 package.json
      const stagedPackage = execSync('git show :package.json', { encoding: 'utf-8' });
      ...
      if (stagedVersion === expectedVersion) {
        ...
        log(COLORS.yellow, `⚠️  版本号已更新为 ${stagedVersion} (预期 ${expectedVersion})`);
